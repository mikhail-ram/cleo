...
module Functions.Weather (weather) where
...
filePath = ".weatherConfig"

weather :: IO ()
weather = do
    config' <- try . readFile $ ".weatherConfig" :: IO (Either IOException String)
    case config' of
      Left _ -> putStrLn $ "Error: Config file path '" ++ filePath ++ "' could not be found.\n"
      Right config' -> do
